...
import { ChartMode } from '../../pages/StatisticsPage';
...
export function StatChart({ selectedChartMode, selectedDate, changeSelectedDate }: StatChartProps) {
  ...
  useEffect(() => {
    ...
    //Высчитываем максимальное время работы на протяжении недели
    maxWorkSec = weekdays.reduce((prev, current) => prev > current.workSec ? prev : current.workSec, 0);

  }, [selectedDate, selectedChartMode, statItems]);
  ...
  const calcHeight = (sec: number, maxSec: number) => {
		let maxHeight = 365;
